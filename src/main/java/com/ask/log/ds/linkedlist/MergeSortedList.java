package com.ask.log.ds.linkedlist;

/**
 * ou are given the heads of two sorted linked lists list1 and list2.
 *
 * Merge the two lists in a one sorted list. The list should be made by splicing together the nodes of the first two lists.
 *
 * Return the head of the merged linked list.
 *
 *
 *
 * Example 1:
 *
 *
 * Input: list1 = [1,2,4], list2 = [1,3,4]
 * Output: [1,1,2,3,4,4]
 * Example 2:
 *
 * Input: list1 = [], list2 = []
 * Output: []
 * Example 3:
 *
 * Input: list1 = [], list2 = [0]
 * Output: [0]
 *
 *
 * Constraints:
 *
 * The number of nodes in both lists is in the range [0, 50].
 * -100 <= Node.val <= 100
 * Both list1 and list2 are sorted in non-decreasing order.
 */
public class MergeSortedList {
    public static void main(String[] args) {
        ListNode node1 = new ListNode(1);
        node1.next = new ListNode(2);
        node1.next.next = new ListNode(4);

        ListNode node2 = new ListNode(1);
        node2.next = new ListNode(3);
        node2.next.next = new ListNode(4);

        MergeSortedList mergeSortedList = new MergeSortedList();
        ListNode result = mergeSortedList.mergeTwoLists(node1, node2);
    }

    public ListNode mergeTwoLists(ListNode list1, ListNode list2) {

        ListNode resultNode = null;
        ListNode tailNode = null;
        ListNode leftNode = list1;
        ListNode rightNode = list2;
       while (leftNode != null && rightNode != null) {
           if (leftNode.val <= rightNode.val) {
               ListNode temp = copyVal(leftNode);
               if (resultNode == null) {
                   resultNode = temp;
                   tailNode = temp;
               } else {
                   tailNode.next = temp;
                   tailNode = temp;
               }
               leftNode = leftNode.next;
           } else {
               ListNode temp = copyVal(rightNode);
               if (resultNode == null) {
                   resultNode = temp;
                   tailNode = temp;
               } else {
                   tailNode.next = temp;
                   tailNode = temp;
               }

               rightNode = rightNode.next;
           }

       }
       while (leftNode != null) {
           ListNode temp = copyVal(leftNode);
           if (resultNode == null) {
               resultNode = temp;
               tailNode = temp;
           } else {
               tailNode.next = temp;
               tailNode = temp;
           }
           leftNode = leftNode.next;
       }
        while (rightNode != null) {
            ListNode temp = copyVal(rightNode);
            if (resultNode == null) {
                resultNode = temp;
                tailNode = temp;
            } else {
                tailNode.next = temp;
                tailNode = temp;
            }
            rightNode = rightNode.next;
        }
       return resultNode;
    }

    private ListNode copyVal(final ListNode node) {
        return new ListNode(node.val);
    }
}
